...
class BlazeLandmark(BlazeBase):
    ...
    def denormalize_landmarks(self, landmarks, affines):
        ...
        gpu = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        affines=torch.tensor(affines,device=gpu)
        ...
        for i in range(len(landmarks)):
            ...
            #print(landmark.data)
            #print(affine.data)
            #print(landmarks.is_cuda)
            #print(landmark.is_cuda)
...
class BlazeDetector(BlazeBase):
    ...
    def detection2roi(self, detection):
        ...
        theta1 = np.arctan2((y0-y1).cpu(), (x0-x1).cpu()) - self.theta0
        theta2=torch.tensor(theta1,device=gpu)
        xc2=torch.tensor(xc,device=gpu)
        yc2=torch.tensor(yc,device=gpu)
        scale2=torch.tensor(scale,device=gpu)
        #print(type(theta1))
        #print(theta1.is_cuda)
        #print(theta2.is_cuda)
        #print(xc.is_cuda)
        #print(yc.is_cuda)
        #print(scale2.is_cuda)
        return xc2, yc2, scale2, theta2
